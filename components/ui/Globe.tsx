...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  // ✅ Fix: use a boolean ref to track initialization
  ...
  useEffect(() => {
    // ✅ Fix: use isInitializedRef.current instead of isInitialized
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      // ✅ Fix: Math.random() is fine inside a callback (not called during render)
...
export function World(props: WorldProps) {
  ...
  // ✅ Fix: guard against SSR — Canvas/WebGL is client-only
  const [mounted, setMounted] = useState(false);
  useEffect(() => {
    setMounted(true);
  }, []);

  // ✅ Fix: memoize Scene and Camera so they are not recreated on every render
  const scene = useMemo(() => {
    const s = new Scene();
